...
if __name__ == "__main__":
    ...
    models=[m.lower().replace("/", "_") for m in ["meta-llama/Llama-3.1-8B-Instruct", "mistralai/Mistral-Small-24B-Instruct-2501", "meta-llama/Llama-3.1-70B-Instruct"]]
    tp_degrees=[1,2,4]
    req_prof_data = {}
    step_prof_data = {}

    data_folder=os.path.abspath("../output/offline/flexllm/profiling")
    

    for i,model in enumerate(models):
        tp_degree=tp_degrees[i]

        fp_req=os.path.join(data_folder, f"inference_request_profiling_sharegpt_{model}_tensor_parallelism_{tp_degree}_max_requests_per_batch_256_max_tokens_per_batch_256_num_kv_cache_slots_300000_qps_0.000000_num_warmup_requests_10.csv")
        req_prof_data[model] = pd.read_csv(fp_req)
        fp_step=os.path.join(data_folder, f"step_profiling_sharegpt_{model}_tensor_parallelism_{tp_degree}_max_requests_per_batch_256_max_tokens_per_batch_256_num_kv_cache_slots_300000_qps_0.000000_num_warmup_requests_10.csv")
